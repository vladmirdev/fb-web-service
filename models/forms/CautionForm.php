...
class CautionForm extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['categories', 'herbs'], 'safe']
        ];
    }

    /**
     * Save caution and create/update related records
     *
     * @return bool
     *
     * @throws Exception
    ...
    public function save()
    {
        ...
        if ($this->id)
            $model = $this->loadCaution($this->id, Actions::UPDATE);

        $transaction = Caution::getDb()->beginTransaction();
                    ...
                    switch ($attribute) {
                        ...
                        case 'categories':
                            ...
                            if(!$model->isNewRecord && (!$value || sizeof($value) == 0)) {
                                ...
                                $excluded = CautionCategory::find()
                                    ->where(['caution_id' => $model->id, 'is_deleted' => 0, 'created_by' => $model->created_by])
                                    ->all();

                                if(sizeof($excluded) > 0) {

                                    /** @var CautionCategory $record */
                            ...
                            foreach ($relations as $relation) {
                                ...
                                if (!isset($relation['id'])) {
                                    ...
                                    $_model->type = Caution::ITEM_TYPE;

                                    if ($_model->save()) {

                                        $ids[] = $_model->id;

                                        $_relation = new CautionCategory();

                                        $_relation->caution_id = $model->id;
                                    ...
                                    $_relation = CautionCategory::findOne(['caution_id' => $model->id, 'category_id' => $relation['id'], 'is_deleted' => 0, 'created_by' => $model->created_by]);
                        ...
                        case 'herbs':
                            ...
                            foreach ($relations as $relation) {
                                ...
                                if (!isset($relation['id'])) {
                                    ...
                                    $_relation = HerbCaution::findOne(['caution_id' => $model->id, 'herb_id' => $relation['id'], 'is_deleted' => 0, 'created_by' => $model->created_by]);
